...
# Load elastica arm without obstacles
arm_without_obstacle_data = np.load("arm_data_without_obstacles.npz")
arm_without_obstacles_position = arm_without_obstacle_data["position_rod"]
arm_without_obstacles_radius = arm_without_obstacle_data["radii_rod"]
arm_without_obstacle_n_elem =  arm_without_obstacle_data["n_elems_rod"]
...
offset_first_arm = 0.25#-0.5 # offset first arm towards the camera which is negative direction
offset_second_arm = -0.5#-0.25#0.5
offset_obstacle = 1.0#-0.5
time_elapse=False
video=True
combine=False
all_octopus=False
...
# time = np.where(np.isnan(arm_without_obstacles_position)==True)[0][0]
# time = 80#[20, 30, 40, 50, 60]
time=210
...
if time_elapse==True:
    ...
    # for k in [38]:
        ...
        for i in range(0,arm_n_elem):
            ...
            file1.writelines(
                ",\n<%f,%f,%f>,%f" % (scale*arm_position[k][0][i] , scale*(arm_position[k][1][i]+offset_first_arm), scale*arm_position[k][2][i], scale*arm_radius[k][i]))
        file1.writelines("\ntexture{")
        ...
        for i in range(0, 1):
            file1.writelines(
                "\n<%f,%f,%f>,%f" % (scale * arm_position[k][0][i] , scale * (arm_position[k][1][i]+offset_first_arm) ,
        ...
        # file1.writelines("\ntexture{")
        # file1.writelines("pigment{ color rgb<1,0.5,0.4>  transmit %f }" % (0.4))
        # file1.writelines("finish{ phong 1 } }")
        # file1.writelines("scale<4,4,4> rotate<0,90,90> translate<2,0,4>    }\n")
        ...
        # for j in obstacles_to_be_plotted:
            ...
            for i in range(0, number_of_elem_obstacles):
                file1.writelines(
                    ",\n<%f,%f,%f>,%f" % (
                    scale * obstacle_position[0][i][j], scale * (0.5*(obstacle_position[1][i][j] + offset_obstacle)), scale * obstacle_position[2][i][j],
                    scale * obstacle_radius[j]))
            file1.writelines("\ntexture{")
            file1.writelines("pigment{ color LimeGreen  transmit %f }" % (0.1)) #0.7
            file1.writelines("finish{ phong 1 } }")
            file1.writelines("scale<4,4,4> rotate<0,90,90> translate<2,0,4>    }\n")

        if k <= 80:

            # Arm without obstacles
            file1.writelines("sphere_sweep\n{b_spline %d" % int(arm_n_elem))
            for i in range(0, arm_n_elem):
                file1.writelines(
                    ",\n<%f,%f,%f>,%f" % (
                    scale * arm_without_obstacles_position[k][0][i], scale * (arm_without_obstacles_position[k][1][i]+offset_second_arm), scale * (arm_without_obstacles_position[k][2][i]),
                    scale * arm_without_obstacles_radius[k][i]))
            file1.writelines("\ntexture{")
            ...
            for i in range(0, 1):
                file1.writelines(
                    "\n<%f,%f,%f>,%f" % (scale * arm_without_obstacles_position[k][0][i], scale * (arm_without_obstacles_position[k][1][i]+offset_second_arm),
                                         scale * arm_without_obstacles_position[k][2][i], 1.5 * scale * arm_without_obstacles_radius[k][i]))
            file1.writelines("\ntexture{")
            file1.writelines("pigment{ color rgb<0.75,0.75,0.75>  transmit %f }" % (0.1))
            file1.writelines("finish{ phong 1 } }")
            file1.writelines("scale<4,4,4> rotate<0,90,90> translate<2,0,4>    }\n")
            ...
            for i in range(0, arm_n_elem):
                file1.writelines(
                    ",\n<%f,%f,%f>,%f" % (
                        scale * arm_without_obstacles_position[80][0][i],
                        scale * (arm_without_obstacles_position[80][1][i] + offset_second_arm),
                        scale * (arm_without_obstacles_position[80][2][i]),
                        scale * arm_without_obstacles_radius[80][i]))
            file1.writelines("\ntexture{")
            file1.writelines("pigment{ color Orange  transmit %f }" % (0.1))
            file1.writelines("finish{ phong 1 } }")
            file1.writelines("scale<4,4,4> rotate<0,90,90> translate<2,0,4>    }\n")
            ...
            for i in range(0, 1):
                file1.writelines(
                    "\n<%f,%f,%f>,%f" % (scale * arm_without_obstacles_position[80][0][i],
                                         scale * (arm_without_obstacles_position[80][1][i] + offset_second_arm),
                                         scale * arm_without_obstacles_position[80][2][i],
                                         1.5 * scale * arm_without_obstacles_radius[80][i]))
            file1.writelines("\ntexture{")
            file1.writelines("pigment{ color rgb<0.75,0.75,0.75>  transmit %f }" % (0.1))
            file1.writelines("finish{ phong 1 } }")
            file1.writelines("scale<4,4,4> rotate<0,90,90> translate<2,0,4>    }\n")
...
#     os.system("ffmpeg -threads 8 -r 20 -i ./high_resolution/combine_arm%02d.png -b:v 90M -vcodec mpeg4 ./high_resolution/0.mp4")
    ...
    os.system(
        "ffmpeg -threads 8 -r 60 -i ./images/moving_arm%02d.png -b:v 90M -vcodec mpeg4 ./images/0.mp4")
