...
class Environment(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.filename_obstacles = kwargs.get("filename_obstacles","obstacle_information_nest_11.npz")
        if GENERATE_NEW_OBSTACLES == True:
            ...
            for i in range(self.N_OBSTACLE):
                ...
                direction = np.array([np.cos(alpha)*np.cos(beta), -np.sin(alpha), np.cos(alpha)*np.sin(beta)])
                ...
                start[1] = self.target_position[1] - (0.5 * self.obstacle_length[i] * self.obstacle_direction[i])[1]
    ...
    def reset(self):
        ...
        for i in range(self.N_OBSTACLE):
            ...
            for k in range(0, 3):
                position_collection_for_plotting[k, ...] = np.linspace(self.obstacle_start[i][k], end[k], n_elem_for_plotting)
