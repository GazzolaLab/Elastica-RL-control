...
from single_arm_basis_postprocessing_POST import (
...
class Environment(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.filename_obstacles = kwargs.get(
            "filename_obstacles", "new_obstacles.npz"
        )
    ...
    def reset(self):
        ...
        self.obstacle_histories = [defaultdict(list) for _ in range(self.N_OBSTACLE_ALL)]
        ...
        for i in range(self.N_OBSTACLE):
            # if i == 7:
            #     self.obstacle_start[i][2] = self.obstacle_start[i][2] - 0.15
            # if i == 6:
            #     self.obstacle_start[i][2] = self.obstacle_start[i][2] - 0.25
            # if i == 5:
            #     self.obstacle_start[i][0] = self.obstacle_start[i][0] + 0.075
            #     alpha = 0.4 * np.pi
            #     beta=0.7*np.pi
            #     direction = np.array(
            #     [np.cos(alpha) * np.cos(beta),-np.sin(alpha),np.cos(alpha) * np.sin(beta),])
            #     direction /= np.linalg.norm(direction)
            #     normal = np.array([np.sin(alpha) * np.cos(beta),np.cos(alpha),np.sin(alpha) * np.sin(beta),])
            #     normal /= np.linalg.norm(normal)
            #     self.obstacle_direction[i] = direction.copy()
            #     self.obstacle_normal[i] = normal.copy()
            ...
            self.simulator.connect(self.shearable_rod, self.obstacle[i]).using(
                ExternalContact, k=2*8e4, nu=4.0
            )


        # def generate_new_obstacle(alpha, beta, length, radii, start, i):
        ...
        #     )
        #     normal /= np.linalg.norm(normal)
        #     new_obstacle_direction = direction.copy()
        #     new_obstacle_normal = normal.copy()

        #     # start = np.zeros((3))
        #     # start[0] = np.random.uniform(nest_start_pos_x, nest_end_pos_x)
        #     # start[1] = (
        #     #     self.target_position[1]
        #     #     - (0.5 * length * new_obstacle_direction)[1]
        #     # )
        #     # start[2] = np.random.uniform(nest_start_pos_z, nest_end_pos_z)

        #     new_obstacle_start = start

        #     self.obstacle[i] = Cylinder(
        #         start=new_obstacle_start,
        #         direction=new_obstacle_direction,
        #         normal=new_obstacle_normal,
        #         base_length=length,
        #         base_radius=radii,
        #         density=1000,
        #     )
        ...
        #     ] = (
        #         start.reshape(3, 1)
        #         + new_obstacle_direction.reshape(3, 1)
        #         * np.linspace(
        #             0, length, self.number_of_points_on_cylinder
        #         )
        #     ).T

        #     self.obstacle_histories[i]["radius"] = radii
        #     self.obstacle_histories[i]["height"] = length
        #     self.obstacle_histories[i]["direction"] = new_obstacle_direction.copy()
        ...
        #     end = (
        #         new_obstacle_start
        #         + new_obstacle_direction * length
        #     )
        #     for k in range(0, 3):
        #         position_collection_for_plotting[k, ...] = np.linspace(
        #             new_obstacle_start[k], end[k], n_elem_for_plotting
        #         )
        ...
        # length, radii = self.obstacle_length[i], self.obstacle_radii[i]
        # start = [ [-0.8,0.1,0.7], [-0.7,0.8,0.8] ]
        # alpha = [-np.pi/10, 0]
        # beta = [-np.pi/3.5, -np.pi/2.5]
        # for j in range(self.num_new_obstacle):
        #     i = j + self.N_OBSTACLE
        #     # alpha = np.pi/2
        #     # beta = np.pi/2 #np.random.uniform(np.pi / 3, np.pi * 2 / 3)
        #     generate_new_obstacle(alpha[j], beta[j], length, radii, np.array(start[j]), i)
        ...
        # np.savez(
        #     os.path.join(save_folder, "new_obstacles"),
        #     N_OBSTACLE=self.N_OBSTACLE,
        #     obstacle_direction=self.obstacle_direction,
        #     obstacle_normal=self.obstacle_normal,
        #     obstacle_length=self.obstacle_length,
    ...
    def step(self, action):
        ...
        if self.current_step >= self.total_learning_steps:
            ...
            if reward > 0:
                print(" Reward greater than 0! Reward: %0.2f, Distance: %0.2f" % (reward,dist))
            else:
                print(" Finished simulation. Reward: %0.2f, Distance: %0.2f" % (reward,dist))
        """ Done is a boolean to reset the environment before episode is completed """
        ...
        if invalid_values_condition_state == True:
            ...
            reward = -100
            state[np.argwhere(np.isnan(state))] = self.state_buffer[np.argwhere(np.isnan(state))]
            done = True

        self.state_buffer = state #hold onto state data in case simulation blows up next step
