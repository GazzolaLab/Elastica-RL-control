__doc__ = """This file is for setting an environment for arm going around structured obstacles and reaching to a 
...
class Environment(gym.Env):
    ...
    arm (Cosserat rod), target (rigid sphere), and structured obstacles (rigid cylinders).
    ...
    or twist the arm and arm goes through structured obstacles opening and towards the target. In case arm
    ...
    def __init__(
        ...
        mode=1,
        num_obstacles=0,
        COLLECT_CONTROL_POINTS_DATA=False,
        *args,
        **kwargs,
    ):
        """
        ...
        **kwargs
        ...
        # Collect control points time-history for reproducing the experiment later on.
        self.COLLECT_CONTROL_POINTS_DATA = COLLECT_CONTROL_POINTS_DATA
        if self.COLLECT_CONTROL_POINTS_DATA == True:
            if self.dim == 2.0:
                self.control_point_history_array = np.zeros(
                    (self.total_learning_steps, self.number_of_control_points)
                )
            if self.dim == 3.0 or self.dim == 2.5:
                self.control_point_history_array = np.zeros(
                    (self.total_learning_steps, 2 * self.number_of_control_points)
                )
            if self.dim == 3.5:
                self.control_point_history_array = np.zeros(
                    (self.total_learning_steps, 3 * self.number_of_control_points)
                )
    ...
    def reset(self):
        ...
        if self.N_OBSTACLE > 0:
            ...
            # first obstacle
            start_position_array[0, 0] = com_target[0] + 0.1 + self.obstacle_radii[0]
            start_position_array[1, 0] = com_target[1] - 0.5 * self.obstacle_length[0]
            start_position_array[2, 0] = com_target[2] - 0.3 * self.obstacle_radii[0]

            # second obstacle
            start_position_array[0, 1] = (
                com_target[0] + 0.1 + 3 * self.obstacle_radii[1] + 0.2
            )
            start_position_array[1, 1] = com_target[1] - 0.5 * self.obstacle_length[1]
            start_position_array[2, 1] = com_target[2] - 0.3 * self.obstacle_radii[1]

            # third obstacle
            start_position_array[0, 2] = (
                com_target[0] + 0.1 + 2 * self.obstacle_radii[2] + 0.1
            )
            start_position_array[1, 2] = com_target[1] - 0.5 * self.obstacle_length[2]
            start_position_array[2, 2] = com_target[2] + 2.2 * self.obstacle_radii[2]
            ...
            start_position_array[2, 3] = (
                start_position_array[2, 1]
            ...
            start_position_array[2, 6] = (
                start_position_array[2, 2]
                + self.obstacle_radii[1]
            ...
            for i in range(self.N_OBSTACLE):
                ...
                self.obstacle_histories[i]["direction"] = obstacle_direction.copy()
                ...
                for k in range(0, 3):
                    position_collection_for_plotting[k, ...] = np.linspace(
                        start_position_array[..., i][k], end[k], n_elem_for_plotting
                    )
                ...
                )  # for rendering and plotting k=2*8e4, nu=4.0
        ...
        **kwargs
        ...
        if self.COLLECT_DATA_FOR_POSTPROCESSING:
            ...
            if SAVE_DATA == True:
                ...
                # Save stored control points into a file, to later reproduce the results.
                if self.COLLECT_CONTROL_POINTS_DATA == True:
                    np.savez(
                        os.path.join(save_folder, "arm_control_points_history.npz"),
                        control_point_history=self.control_point_history_array,
                    )
